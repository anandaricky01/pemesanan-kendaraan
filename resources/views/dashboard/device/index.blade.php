...
<form method="get" action="{{ route('dashboard.device.index') }}" class="mb-5">
    <div class="">
...
<div class="col-span-2 relative overflow-x-auto shadow-md sm:rounded-lg border">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <caption class="p-5 text-lg font-semibold text-left text-gray-900 bg-slate-50 dark:text-white dark:bg-gray-800">
            Data Device
            <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">Data Device IoT dapat ditambah, diubah, atau dihapus oleh admin</p>
        </caption>
            ...
            @if($devices->count() > 0)
                @foreach ($devices as $item)
                    <tr class="bg-slate-50 border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('dashboard.device.show', $item->id) }}">
                                <button
                                    class="font-medium text-white bg-blue-500 hover:bg-blue-700 px-2 rounded">
                                    Lihat
                                </button>
                            </a>
                            <a href="{{ route('dashboard.device.edit', $item->id) }}">
                                <button
                            ...
                            <form id="hapus-laporan" action="{{ route('dashboard.device.delete', $item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="p-6 text-center">
                                                ...
                                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                    Yakin ingin menghapus Device {{ $item->name }}?</h3>

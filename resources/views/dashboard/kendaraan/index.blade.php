        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <caption
                ...
                <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                    Data kendaraan dapat ditambah, dirubah, dan dihapus oleh admin
                </p>
            </caption>
            ...
            <tbody>
                ...
                @foreach ($kendaraan->sortBy('status') as $ken)
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <td>
                        <a href="{{ route('dashboard.kendaraan.show', $ken->id) }}">
                            <button
                                class="font-medium text-white bg-blue-500 hover:bg-blue-700 px-2 rounded">
                                Lihat
                            </button>
                        </a>
                        <a href="{{ route('dashboard.kendaraan.edit', $ken->id) }}">
                            <button
                                class="font-medium text-white bg-yellow-300 hover:bg-yellow-500 px-2 rounded">
                                Edit
                            </button>
                        </a>
                        <button data-modal-target="popup-modal-{{ $ken->id }}" data-modal-toggle="popup-modal-{{ $ken->id }}"
                            class="font-medium text-white bg-red-500 hover:bg-red-700 px-2 rounded">
                            Hapus
                        </button>
                        <form id="hapus-kendaraan" action="{{ route('dashboard.kendaraan.delete', $ken->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <div id="popup-modal-{{ $ken->id }}" tabindex="-1"
                                class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
                                <div class="relative w-full h-full max-w-md md:h-auto">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button type="button"
                                            class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                                            data-modal-hide="popup-modal-{{ $ken->id }}">
                                        ...
                                        <div class="p-6 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                Yakin ingin menghapus kendaraan {{ $ken->title }}?</h3>
                                            <button data-modal-hide="popup-modal-{{ $ken->id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                Iya
                                            </button>
                                            <button data-modal-hide="popup-modal-{{ $ken->id }}" type="button"
...
<div class="mt-5 text-center">
    {{ $kendaraan->links('pagination::tailwind') }}
</div>
@endsection
